...
namespace LD45
{
  ...
  void Game::Update()
  {
    ...
    for (auto& scene : _scenes)
    {
      if (scene.second != _currentScene && scene.second->GetResetWanted())
      {
        scene.second = scene.second->DoReset();
      }
    }
  ...
  void Game::UpdateTextObject(std::unique_ptr<LD45::ITextObject>& textObject)
  {
  ...
  size_t Game::RandomNumber(size_t min, size_t max)
  {
