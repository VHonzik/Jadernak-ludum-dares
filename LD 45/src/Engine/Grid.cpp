...
namespace LD45
{
  Grid::Grid(const uint32_t rows, const uint32_t columns, const int32_t width, const int32_t height)
    : _rows(rows)
    , _columns(columns)
    , _width(width)
    , _height(height)
  ...
  void Grid::SetPosition(int32_t x, int32_t y)
  {
    _x = x;
    _y = y;
    Align();
  ...
  void Grid::SetCenterPosition(int32_t x, int32_t y)
  {
    _x = x - _width / 2;
    _y = y - _height / 2;
    Align();
